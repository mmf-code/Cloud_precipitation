...
plt.legend(legend_labels, loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=len(legend_labels))
...
for i in times:
    ...
    ax = fig.add_subplot(1, 1, 1, projection=ccrs.PlateCarree())  # set the projection

    # Plot the wind vectors with color mapping
    quiver = ax.quiver(lon, lat, u10[i, :, :], v10[i, :, :], wind_speed[i, :, :], transform=ccrs.PlateCarree(), cmap='twilight', norm=norm)
